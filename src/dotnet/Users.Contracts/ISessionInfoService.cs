...
public interface ISessionInfoService
{
    ...
    Task Update(UpsertCommand command, CancellationToken cancellationToken);

    /// <summary>
    /// Create or update value inside of current <seealso cref="SessionInfo.Options"/>
    /// </summary>
    public record UpsertCommand(Session Session, KeyValuePair<string, string> KeyValue) : ISessionCommand<Unit>;
}