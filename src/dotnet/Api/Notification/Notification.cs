    ...
    ) : IHasId<NotificationId>, IHasVersion<long>, IUnionRecord<NotificationOption?>, ISanitized
{
    ...
    [DataMember(Order = 2), MemoryPackOrder(2), Key(2)] public string Title { get => Sanitizer.MaskPrivate(field); init; } = "";
    [DataMember(Order = 3), MemoryPackOrder(3), Key(3)] public string Content { get => Sanitizer.MaskPrivate(field); init; } = "";
    [DataMember(Order = 4), MemoryPackOrder(4), Key(4)] public string IconUrl { get; init; } = "";
    [DataMember(Order = 5), MemoryPackOrder(5), Key(5)] public Moment CreatedAt { get; init; }
    [DataMember(Order = 6), MemoryPackOrder(6), Key(6)] public Moment SentAt { get; init; }
    [DataMember(Order = 7), MemoryPackIgnore, Key(7)] public Moment? HandledAt { get; init; }
