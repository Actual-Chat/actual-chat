...
class PlaybackToggleSvg extends LitElement {
    ...
    protected render(): unknown {
        if (this.isOn)
            return html`
                <svg class="spin-and-on" width="${this.size}" height="${this.size}" viewBox="${this.viewBox}" fill="${this.isOn ? "var(--primary)" : "var(--round-title)"}" xmlns="http://www.w3.org/2000/svg">
        ...
        if (!this.isAnimated)
            return html`
                <svg width="${this.size}" height="${this.size}" viewBox="${this.viewBox}" fill="var(--round-title)" xmlns="http://www.w3.org/2000/svg">
