...
public sealed class MauiTempFileDownloader(IServiceProvider services)
{
    ...
    {
        var downloadsPath = (FilePath)FileSystem.Current.CacheDirectory & "downloads";
        RemoveStale(downloadsPath);
        ...
        var folder = Directory.CreateDirectory(downloadsPath & RandomStringGenerator.Default.Next());
        var filePath = (FilePath)folder.FullName & GetFileName(fileName, contentType);
        var target = File.OpenWrite(filePath);
        await using var __ = target.ConfigureAwait(false);
        await Copy(source, target, length, progress, cancellationToken).ConfigureAwait(false);
        return filePath;
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        while (true) {
            ...
            copied += read;
            var percent = (int)(100 * copied / length.Value);
    ...
    private void RemoveStale(FilePath downloadsPath)
    {
        try {
            ...
            foreach (var folder in Directory.EnumerateDirectories(downloadsPath)) {
