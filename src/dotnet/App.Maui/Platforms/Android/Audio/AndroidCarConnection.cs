    ...
    private int ReadState()
    {
        ...
        try {
            ...
            if (cursor == null || !cursor.MoveToNext()) {
                ...
                _log.Log(ReadFailureLevel(lastKnownState),
                    "Car connection provider returned {Result}, keeping state {State}",
            ...
            var lastKnownState = Volatile.Read(ref _lastKnownState);
            _log.Log(ReadFailureLevel(lastKnownState), e,
                "Couldn't read the car connection state, keeping state {State}", lastKnownState);
            return lastKnownState;
        }
    }

    private static LogLevel ReadFailureLevel(int lastKnownState)
        // A phone with no Android Auto has no provider at all, and the 30s recheck would warn
        // forever; only a read that may be masking a live projection is worth a warning.
        => lastKnownState == ConnectionTypeProjection ? LogLevel.Warning : LogLevel.Debug;
