...
/// <summary>
/// Routes <see cref="CallScreensUI"/> rings and outgoing <see cref="CallUI"/> calls to
...
public sealed class IosCallsBridge : IIncomingCallsBridge, ISystemCallUI, IDisposable
{
    ...
    private async Task WatchCallEnd(ChatId chatId, CancellationToken cancellationToken)
    {
        ...
        var hasJoined = false;
        try {
            await AsyncChain.From(Watch, $"Call-end watch for chat #{chatId}")
        ...
        async Task Watch(CancellationToken ct) {
            hasJoined = hasJoined
                || await WhenJoined(chatId, JoinTimeout - startedAt.Elapsed, ct)
                    .ConfigureAwait(false);
            if (!hasJoined) {
            ...
            await WhenLeft(chatId, ct).ConfigureAwait(false);
