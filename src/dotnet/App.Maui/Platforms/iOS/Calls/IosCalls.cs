...
/// places, and routes the system call UI's actions back into <see cref="CallScreensUI"/>.
/// A static singleton because a VoIP push routinely starts the process, with no Blazor
/// scope to belong to.
/// </summary>
public sealed class IosCalls : CXProviderDelegate
{
    ...
    public bool ReportOutgoingCallStatus(ChatId chatId, CallerStatus status)
    {
        ...
        case CallerStatus.Active:
            ...
            return true;
        case CallerStatus.NoAnswer:
            // A decline reads the same to the caller - the server reports both as NoAnswer.
    ...
    public override void PerformAnswerCallAction(CXProvider provider, CXAnswerCallAction action)
    {
        ...
        Log.LogInformation("PerformAnswerCallAction: chat #{ChatId}, handledLocally={IsHandledLocally}, app={AppState}",
            call.ChatId, isHandledLocally, UIApplication.SharedApplication.ApplicationState);
        call.MarkAnswered();
        AudioSession.IsCallVideo = call.HasVideo;
        // Configured and claimed before the fulfil, never activated: CallKit activates the session
        // itself and hands it over in DidActivateAudioSession, and it leaves an idle app's Ambient
        // session alone in the background - that callback then never comes, and an answer from the
        // lock screen records nothing.
    ...
    public override void PerformEndCallAction(CXProvider provider, CXEndCallAction action)
    {
        ...
        Log.LogInformation(
            "PerformEndCallAction: chat #{ChatId}, answered={IsAnswered}, handledLocally={IsHandledLocally}",
            call.ChatId, isAnswered, isHandledLocally);
        action.Fulfill();
        if (isHandledLocally)
            return;
    ...
    {
        // The system call screen's Mute button - the only one there is after a lock-screen answer.
        // Failed before the call is live: an unmute there would start recording ahead of the call.
        if (!TryGetCall(action.CallUuid, out var call) || !call.IsAnswered) {
            action.Fail();
            return;
        }

        var chatId = call.ChatId;
        var isMuted = action.Muted;
        action.Fulfill();
        _ = DispatchToBlazor(c => SetMutedLocally(c, chatId, isMuted), "PerformSetMutedCallAction");
    }
    ...
    {
        Log.LogInformation("DidDeactivateAudioSession: owner={Owner}", AudioSession.Owner);
        ReleaseAudioSession();
    }
    ...
    private void RequestTransaction(Guid callId, Call call, CXAction action, LocalAction localAction)
    {
        ...
        // CallScreensUI for something the app itself just did.
