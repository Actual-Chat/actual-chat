...
{
    protected virtual string NotFoundMessage() => "Was not able to find anything";
    public async Task ExecuteAsync(IEnumerable<ChatEntry>? updatedDocuments, IEnumerable<ChatEntry>? deletedDocuments, CancellationToken cancellationToken) {
        ...
        var documentFound = searchResult.Documents.FirstOrDefault();
        var chat = await chats.Get(chatId, cancellationToken).ConfigureAwait(false);

        var text = documentFound == null ? NotFoundMessage() : documentFound.Document.Text;
