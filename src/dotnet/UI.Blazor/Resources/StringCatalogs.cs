...
public static class StringCatalogs
{
    ...
    internal static Dictionary<string, string>? Load(Kind kind, Language language)
    {
        using var stream = Assembly.GetManifestResourceStream(ResourceName(kind, language.IsoCode));
        return stream == null
            ? null
