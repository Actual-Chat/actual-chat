...
public class ConversationsBackend(IServiceProvider services) : DbServiceBase<ChatDbContext>(services), IConversationsBackend
{
    ...
    {
        var tile = ConversationIdTiles.GetTile(range);
        var conversationRangeTile = await GetConversationRangeTile(chatId, tile.Start, cancellationToken)
            .ConfigureAwait(false);
        var conversations = await conversationRangeTile.ConversationIds
            .Distinct()
        ...
        CancellationToken cancellationToken)
    {
        ...
        var conversationRangeTile = await GetConversationRangeTile(
            ...
            .ConfigureAwait(false);
        var existingConversations = conversationRangeTile.ConversationIds;
