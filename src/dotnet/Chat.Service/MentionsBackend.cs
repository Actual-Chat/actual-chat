...
public class MentionsBackend(IServiceProvider services) : DbServiceBase<ChatDbContext>(services), IMentionsBackend
{
    ...
    public virtual async Task OnTextEntryChangedEvent(TextEntryChangedEvent eventCommand, CancellationToken cancellationToken)
    {
        ...
        var toAddMentionIds = ApiArray<MentionId>.Empty;
        ...
        if (changeKind is ChangeKind.Remove) {
            ...
            toAddMentionIds = toAdd.Select(m => new MentionId(m.MentionId)).ToApiArray();
        ...
        if (chatId.IsThread && toAddMentionIds.Count > 0)
