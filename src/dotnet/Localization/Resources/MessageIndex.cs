...
public sealed partial class MessageIndex
{
    ...
    public MessageIndex(IReadOnlyDictionary<string, string> messages)
    {
        ...
        foreach (var (key, message) in messages) {
            ...
            // Add, not TryAdd: two keys sharing one English value make the reverse lookup
            // ambiguous, and picking either would mislocalize the other.
            if (!PlaceholderRe.IsMatch(message)) {
                _keyByMessage.Add(message, key);
                continue;
            }

            keyByTemplate.Add(message, key);
