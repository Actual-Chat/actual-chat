...
namespace Markdig.Helpers
{
    ...
    internal sealed class CompactPrefixTree<TValue> : IReadOnlyDictionary<string, TValue>, IReadOnlyList<KeyValuePair<string, TValue>>
    {
        ...
        private Node[] _tree;
        private static readonly Node[] _emptyTree = Array.Empty<Node>();

        private KeyValuePair<string, TValue>[] _matches;
        private static readonly KeyValuePair<string, TValue>[] _emptyMatches = Array.Empty<KeyValuePair<string, TValue>>();

        private int _childrenIndex = 0;
        private int[] _children = _emptyChildren;
        private static readonly int[] _emptyChildren = Array.Empty<int>();
        ...
        private bool TryInsert(in KeyValuePair<string, TValue> pair, InsertionBehavior behavior)
        {
            ...
            if (TryGetRoot(rootChar, out int rootNodeIndex))
            {
                    ...
                    if (node.Children == -1)
                    {
                        ...
                        if (node.ChildChar == 0)
                        {
                            ...
                            if (i == minLength && key.Length == previousKey.Length)
                            {
                                ...
                                Debug.Assert(string.Equals(key, previousKey, StringComparison.Ordinal));
                ...
                if (node.MatchIndex != -1)
                {
                    ...
                    {
                        Debug.Assert(string.Equals(previousMatch.Key, key, StringComparison.Ordinal));
                        goto HandleDuplicateKey;
                    }
                    else
                    {
            ...
            HandleDuplicateKey:;
                Debug.Assert(string.Equals(key, _matches[node.MatchIndex].Key, StringComparison.Ordinal));
