    ...
    ILogger<ApnsClient> log) : IApnsClient
{
    ...
    private bool _isPttConfigWarningLogged;
    private bool _isVoipConfigWarningLogged;
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!IsConfigured) {
            // Guard flag: at worst the warning is logged twice.
            if (!Volatile.Read(ref _isPttConfigWarningLogged)) {
                Volatile.Write(ref _isPttConfigWarningLogged, true);
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!IsConfigured) {
            if (!Volatile.Read(ref _isVoipConfigWarningLogged)) {
                Volatile.Write(ref _isVoipConfigWarningLogged, true);
