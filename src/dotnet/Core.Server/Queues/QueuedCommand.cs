...
{
    private static readonly MethodInfo NewMethod = typeof(QueuedCommand)
        .GetMethod(nameof(New), BindingFlags.Static | BindingFlags.Public)!;
    ...
    public static QueuedCommand New<T>(T command, Ulid id = default)
        where T : ICommand
    {
        if (typeof(T) != command.GetType())
            return NewUntyped(command, id);

        if (id == default)
            id = Ulid.NewUlid();
        var result = new QueuedCommand<T>(command) { Id = id };
        return result;
    }
    ...
    public static QueuedCommand NewUntyped(ICommand command, Ulid id = default)
        => Factories.GetOrAdd(
            command.GetType(),
            static t => NewMethod.MakeGenericMethod(t).CreateDelegate<Func<ICommand, Ulid, QueuedCommand>>()
        ).Invoke(command, id);
