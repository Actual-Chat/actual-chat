...
public static class ShardKeyResolvers
{
    ...
    {
        Register<ShardKey>(static x => x);
        Register<Unit>(static _ => default);
        Register<int>(ShardKey.New);
        Register<uint>(ShardKey.New);
        Register<string>(ShardKey.New);
        Register<Symbol>(ShardKey.New);
        Register<IHasShardKey>(static x => x.ShardKey);
    ...
    public static void Register<T>(ShardKeyResolver<T> resolver)
    {
        ...
        if (type != typeof(IHasShardKey) && typeof(IHasShardKey).IsAssignableFrom(underlyingType))
    ...
    private sealed class Factory<T> : GenericInstanceFactory, IGenericInstanceFactory<ShardKeyResolver<T>>
    {
        public override ShardKeyResolver<T> Generate()
        {
            ...
            if (typeof(IHasShardKey).IsAssignableFrom(type)) {
                var resolver = (ShardKeyResolver<IHasShardKey>)Registered[typeof(IHasShardKey)];
                return type.IsValueType
                    ? (ShardKeyResolver<T>)GenericInstanceCache.Get(typeof(StructFactory<>), type)!
                    : (ShardKeyResolver<T>)(Delegate)resolver;
            }
    ...
    private sealed class StructFactory<T> : GenericInstanceFactory, IGenericInstanceFactory<ShardKeyResolver<T>>
        where T : struct, IHasShardKey
    {
        public override ShardKeyResolver<T> Generate()
            => static x => x.ShardKey;
    }
