...
public static class DefaultPicture
{
    ...
    public static string Get(Account account, int size = DefaultSize)
        => GetGravatar(account, size) ?? GetAvataaar(account.User.Name.GetMD5HashCode());

    public static string Get(Account? account, string hash, int size = DefaultSize)
    ...
    public static string? GetGravatar(Account? account, int size = DefaultSize)
