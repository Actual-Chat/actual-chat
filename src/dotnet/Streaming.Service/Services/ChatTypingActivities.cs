...
public class ChatTypingActivities(IServiceProvider services) : IChatTypingActivities
{
    ...
    public virtual async Task SetTyping(
        Session session, ChatId chatId, TypingActivityKind kind, CancellationToken cancellationToken)
    {
        ...
        await Backend.SetTyping(chatId, author.Id, kind, cancellationToken).ConfigureAwait(false);
    }
}
