...
import codec, { Codec } from '@actual-chat/codec/codec.debug';
...
export class AudioPlayerTestPage {
    ...
    public static async create(blazorRef: DotNet.DotNetObject): Promise<AudioPlayerTestPage> {
        const player = await AudioPlayer.create(blazorRef, '0', 312, 'test', 'test', null, 0, 0);
    ...
    public testDecoder() {
        const decoder = new this.codecModule.Decoder(AUDIO.play.sampleRate as 48000 | 16000);
