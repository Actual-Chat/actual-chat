...
export async function detectGPUBackends(): Promise<GPUBackendSupport> {
    const support: GPUBackendSupport = {
        webgpu: false,
        webgl: false,
        wasm: true, // CPU fallback always available
        recommended: 'wasm',
        details: {}
    };
    ...
    try {
        if ('gpu' in navigator) {
            ...
            if (adapter) {
                // Don't allocate a GPUDevice here — it wastes GPU memory iOS Safari
                // needs for the worker, causing OOM when ONNX Runtime requests its own.
    ...
    // WebGPU > WebGL > WASM by performance.
    if (support.webgpu) {
