...
public class SelectionUI : UIServiceBase<AppUIHub>
{
    ...
    public async Task Delete(IReadOnlySet<ChatEntryId>? selection = null) {
        ...
        if (otherAuthorCount > 0) {
            var isOne = otherAuthorCount == 1;
            var messages = isOne ? L.Selection_Messages_One(otherAuthorCount) : L.Selection_Messages_Other(otherAuthorCount);
            var confirmed = false;
            var model = new ConfirmModal.Model(true,
                L.Selection_DeleteOthersConfirm_Format(messages),
                () => { confirmed = true; }) {
                Title = isOne ? L.Selection_DeleteTitle_One : L.Selection_DeleteTitle_Other,
    ...
    public async Task Forward(IReadOnlySet<ChatEntryId>? selection = null) {
        ...
        {
            var messages = selection.Count == 1
                ? L.Selection_Messages_One(selection.Count)
                : L.Selection_Messages_Other(selection.Count);
