...
public sealed class ChunkedFileUploader(AppUIHub hub) : UIServiceBase<AppUIHub>(hub)
{
    ...
    public FileUploadOperation CreateUploadOperation(Task whenFileStreamReady, UploadId uploadId, Task<Stream> getStream)
    {
        var progress = new UploadProgressTracker();
        return new FileUploadOperation(whenFileStreamReady, async token => {
