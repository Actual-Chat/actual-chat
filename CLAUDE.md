...
**Running the server**: Use `/server-start` to start the server, `/server-restart` to rebuild and restart, `/server-stop` to stop. Use `--watch` flag for auto-reload during UI development. See the "Autonomous UI Development Loop" section in `AGENTS.md` for the full workflow.
