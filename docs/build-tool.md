...
`<platform>` is `android`, `ios`, `windows` or `macos`.
...
These delegate to `scripts/run-ios.sh`, `scripts/run-ios-simulator.sh` and
`scripts/run-macos.sh`, which handle device detection, certificate install and
...
publish-android      publish-ios         publish-maccatalyst publish-win
restore              restore-tools       slnf                slow-tests
tests                unit-tests          watch
```
...
│   │   ├── <PLATFORM> (Android|Ios|Windows|MacOs)  android | ios | windows | macos
