...
|---|---|---|
| Ship only `woff2` fonts | re-measure | woff2 works in WKWebView (iOS 10+), Chromium (Android WebView, WebView2) and every browser since 2016 — well below our floors of iOS 16.4 / API 28 / Windows 10 17763. The icon font is done: `.svgtofontrc` sets `excludeFormat`, so `npm run font` emits woff2 only, and 770 KB of `eot`/`woff`/`ttf`/`svg` stopped reaching `dist`. Two chains are left, both still copied by esbuild because their CSS names them: TT Commons Pro (`otf` + `ttf` per face, 2.3 MB uncompressed) and fork-awesome's own `fork-awesome.min.css` in `node_modules` (`eot`/`woff`/`ttf`/`svg`, 1.1 MB). The 1.8 MB above was APK-compressed and predates the icon-font cut — re-measure before quoting it. |
