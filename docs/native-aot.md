...
- `[MessagePackFormatter(typeof(X))]` attributes are read directly; if `X` is an open generic with one parameter, it is also closed over the decorated type (catches e.g. `ObjectIdMessagePackFormatter<ChatId>` for all ~30 identifier types).
