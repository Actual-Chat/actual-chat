...
public class ChatSerializationTests(ITestOutputHelper @out) : TestBase(@out)
{
    ...
    public void ImmutableOptionSetTest1()
    {
        ...
        for (var i = 0; i < 10_000; i++) {
            ...
            foreach (var item in items)
                s = s.Set(item, item);
            var data = Serializers.MemoryPack.Write(s, typeof(ImmutableOptionSet)).WrittenSpan.ToArray();
            if (lastData != null)
    ...
    public void ImmutableOptionSetTest2()
    {
        ...
        for (var i = 0; i < 2; i++) {
            ...
            foreach (var item in items)
                s = s.Set(item, item);
            var data = Serializers.MemoryPack.Write(s, typeof(string[])).WrittenSpan.ToArray();
            if (lastData != null)
                data.SequenceEqual(lastData).Should().BeTrue();
            lastData = data;
        }
