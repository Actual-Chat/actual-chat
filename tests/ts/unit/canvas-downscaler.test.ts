...
describe('CanvasDownscaler.pickSource', () => {
    ...
    it('top layer always uses the original input', () => {
        ...
        expect(CanvasDownscaler.pickSource(2, l, [])).toEqual({ kind: 'original' });
        // Even with bookkeeping (shouldn't matter for top tier).
        expect(CanvasDownscaler.pickSource(2, l, [2])).toEqual({ kind: 'original' });
    });
    ...
    it('lower tier uses original when no higher tier qualifies (≥ 2× rule)', () => {
        ...
        expect(CanvasDownscaler.pickSource(0, l, [1])).toEqual({ kind: 'original' });
    });
    ...
    it('lower tier uses higher tier when long-edge ≥ 2×', () => {
        ...
        expect(CanvasDownscaler.pickSource(0, l, [1, 2])).toEqual({ kind: 'higher', idx: 2 });
    });
    ...
    it('picks the smallest qualifying higher tier (least drawImage work)', () => {
        ...
        //   Pick smallest: L1.
        const l = ladder([320, 180], [640, 360], [1280, 720]);
        expect(CanvasDownscaler.pickSource(0, l, [1, 2])).toEqual({ kind: 'higher', idx: 1 });
    });
    ...
    it('uses original when only painted higher tiers all fail the threshold', () => {
        ...
        expect(CanvasDownscaler.pickSource(0, l, [1, 2])).toEqual({ kind: 'original' });
    });

    it('threshold is exact (≥ 2× passes; < 2× fails)', () => {
        // L0=400, L1=800 → 800/400 = 2 exactly, qualifies.
        const l1 = ladder([400, 225], [800, 450]);
        expect(CanvasDownscaler.pickSource(0, l1, [1])).toEqual({ kind: 'higher', idx: 1 });

        // L0=401, L1=800 → 800/401 ≈ 1.995, fails.
        const l2 = ladder([401, 226], [800, 450]);
        expect(CanvasDownscaler.pickSource(0, l2, [1])).toEqual({ kind: 'original' });
    });
    ...
    it('only considers tiers actually painted (paintedHigher set)', () => {
        ...
        const l = ladder([320, 180], [640, 360], [1280, 720]);
        expect(CanvasDownscaler.pickSource(0, l, [2])).toEqual({ kind: 'higher', idx: 2 });
    });
    ...
    it('walkthrough from the design discussion: L0=320, L1=480, L2=720, original=1080', () => {
        ...
        expect(CanvasDownscaler.pickSource(1, l, [2])).toEqual({ kind: 'original' });
