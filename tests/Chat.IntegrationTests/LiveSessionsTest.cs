    ...
    : SharedAppHostTestBase<AppHostFixture>(fixture, @out)
{
    ...
    public async Task LiveBlockShouldEnterRangeMetaOnlyAfterLatch()
    {
        ...
        var live = await backend.GetState(chatId, default);
        live.Should().NotBeNull();
        var tileStart = Constants.Chat.RangeIdTileLayer.GetTile(live!.StartEntryLid).Range.Start;
        ...
        var liveTileStart = Constants.Chat.RangeIdTileLayer.GetTile(liveStartLid).Range.Start;
        await ComputedTest.When(async ct => {
    ...
    public async Task LiveBlockShouldEnterTileOnlyAfterLatch()
    {
        ...
        await backend.OnStreamRegistered(chatId, author!.Id, null, true, true, default);
        var live = await backend.GetState(chatId, default);
        var tileRange = Constants.Chat.RangeIdTileLayer.GetTile(live!.StartEntryLid).Range;
    ...
    public async Task RangeMetaShouldKeepPreLatchConversationsVisible()
    {
        ...
        // act
        var idTileStart = Constants.Chat.RangeIdTileLayer.GetTile(e0.LocalId).Range.Start;
