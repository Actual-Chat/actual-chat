    ...
    : SharedAppHostTestBase<ChatAppHostFixture>(fixture, @out)
{
    ...
    public async Task ExpandedLiveBlockShouldKeepOptimisticTailBeforeItsFooter()
    {
        ...
        var chat = await CreateSettledChat("live-optimistic-tail-test");
        ...
        await liveBackend.OnStreamRegistered(chat.Id, AuthorId.New(chat.Id, 777_071),
            null, true, true, CancellationToken.None);
        var live = (await liveBackend.GetState(chat.Id, CancellationToken.None))!;
        await Tester.CreateTextEntry(chat.Id, "persisted tail");
        ...
        for (var i = 0; i < 2; i++)
            pending.AddSendingMessage(new SendingMessage(
                Guid.NewGuid().ToString(), "", chat.Id, null, now,
        ...
        try {
            ...
            recorderState.Set(AudioRecorderState.Idle);
        }
    }
    ...
    [Fact]
    public async Task GrowingLiveSummaryShouldOutgrowCachedRangeTiles()
    {
        // arrange
        await Tester.SignInAsUniqueBob();
    ...
    public async Task LaterLiveBlockShouldPreserveTheCollapsedConversationPrefix()
    {
        ...
        var backend = AppHost.Services.GetRequiredService<IConversationsBackend>();
        var tileRange = Constants.Chat.ConversationIdTiles.GetTile(v).Range;
        var chatUI = Tester.ScopedAppServices.GetRequiredService<ChatUI>();
