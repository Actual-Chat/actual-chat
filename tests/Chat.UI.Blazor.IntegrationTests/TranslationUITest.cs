    ...
    : SharedAppHostTestBase<TranslationAppHostFixture>(fixture, @out)
{
    ...
    public async Task ShouldStopTranslatingStreamWhenTranslationIsTurnedOff()
    {
        ...
        var entry = await AliceTester.CreateStreamingEntry(
            chatId, Languages.French, streamId: streamId.Value, cancellationToken: cancellationToken);
    ...
    public async Task ShouldSwitchReaderToSourceStreamWhenTranslationIsTurnedOff()
    {
        ...
        // assert - the reader follows the source
        await AssertTranscriptState(entry.ChatEntrySlim, false, x => x.Text == source.Text);

        // act - the source goes on, so the translated stream keeps producing for whoever still reads it
        var source2 = Text("Bonjour, comment allez-vous ? Je vais bien, merci beaucoup.");
        diffs.Writer.TryWrite(source2 - source);

        // assert - the reader keeps showing the source; a stale translated-stream loop would flip it back
        await AssertTranscriptState(entry.ChatEntrySlim, false, x => x.Text == source2.Text);
        var watch = Stopwatch.StartNew();
        while (watch.Elapsed < TimeSpan.FromSeconds(5)) {
            var state = await TranscriptUI.GetTranscriptState(entry.ChatEntrySlim.Id, cancellationToken);
            state.IsTranslating.Should().BeFalse("state = {0}", state);
            state.Text.Should().Be(source2.Text);
    ...
    private void SetVisibleItems(params IReadOnlyCollection<ChatEntry> entries)
    {
        ...
        ChatUI.ReportItemVisibility(new ChatViewItemVisibility(chatId,
