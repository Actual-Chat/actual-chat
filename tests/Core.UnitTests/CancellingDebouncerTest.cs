...
public class CancellingDebouncerTest
{
    ...
    public async Task ShouldRunManyTimesForRareCalls(int count)
    {
        ...
        for (int i = 1; i <= count; i++) {
            var prev = i - 1;
            await startedEvent.Computed.When(x => x == prev).WaitAsync(interval + TimeSpan.FromSeconds(3));
            sut.Enqueue(i);
        }
    ...
    public async Task ShouldCancelPreviousAlreadyRunning(int count)
    {
        ...
        for (int i = 1; i <= count; i++) {
            sut.Enqueue(i);
            var i1 = i;
            await startedEvent.Computed.When(x => x == i1).WaitAsync(TimeSpan.FromSeconds(3));
        }
