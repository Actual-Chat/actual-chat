...
public class AppLocalizationTest
{
    ...
    public void EveryLocalizerExtMemberResolvesInEveryLanguage()
    {
        ...
        foreach (var language in ShippedLanguages()) {
            var localizer = new TestStringLocalizer(Load(language)!, language);
            foreach (var member in members) {
                ...
                else if (args.OfType<string>().FirstOrDefault(a => !value.Contains(a)) is { } missingArg)
    ...
    [Fact]
    public void EveryPluralKeyHasFormsForItsLanguage()
    {
        // A plural value lists its language's forms separated by '|'. Listing fewer is fine -
        // the index clamps to the last one - but listing more leaves a form unreachable.

        // arrange
        var pluralKeys = LocalizerExtMembers().Where(m => m.IsPlural).Select(m => m.Name).ToList();

        // act
        var errors = new List<string>();
        foreach (var language in ShippedLanguages()) {
            var strings = Load(language)!;
            var maxFormCount = language.GetPluralFormCount();
            foreach (var key in pluralKeys) {
                var formCount = strings[key].Split(FormSeparator).Length;
                if (formCount > maxFormCount)
                    errors.Add($"'{language.IsoCode}.{key}' lists {formCount} forms, not {maxFormCount}");
            }
        }

        // assert
        errors.Should().BeEmpty(
            "a plural value must not list more forms than its language has:\n{0}", string.Join("\n", errors));
    }
    ...
    private static IEnumerable<LocalizerExtMember> LocalizerExtMembers()
    {
        ...
        {
            var implementation = typeof(LocalizedStringsLocalizerExt).GetMethod(implementationName, bf);
            return new LocalizerExtMember(name, implementation.Require(implementationName));
        }
    }
    ...
    {
        // A plural member takes the count first, so it can pick the form - see PluralLocalizerExt
        public bool IsPlural
            => Implementation.GetParameters() is [_, { ParameterType: var t }, ..] && t == typeof(long);

        public object[] NewArgs()
            => Implementation.GetParameters()
                .Skip(1)
                .Select(object (p, i) => p.ParameterType == typeof(long) ? 1L : $"<arg{i}>")
                .ToArray();
