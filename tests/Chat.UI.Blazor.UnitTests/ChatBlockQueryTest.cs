...
public sealed class ChatBlockQueryTest
{
    ...
    [InlineData(true)]
    [InlineData(false)]
    public void ExpandedCoverageShouldRequestMissingRangeMetadata(bool isBefore)
    {
        // arrange
        var block = new Range<long>(100, 3000);
        var meta = new ChatRangeMeta(new(1280, 2560), [new(1280, 2560)], [block], 1,
    ...
    private static ChatRangeMeta NewMeta(Range<long>[] entries, Range<long>[] conversations)
    ...
    {
        var ranges = meta.ConversationLidRanges.Concat(collapsedBlocks)
