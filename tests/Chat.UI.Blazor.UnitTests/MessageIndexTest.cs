...
public class MessageIndexTest
{
    ...
    [Fact]
    public void FieldNamesShouldBeIndexedSeparately()
    {
        // Keeping the two indexes apart is what stops a one-word field name
        // from shadowing a message that happens to read the same.

        // arrange
        var index = New(
            ("Validation_PhoneOrEmail", "Phone or email"),
            ("Field_PhoneOrEmail", "Phone or email"));

        // act
        var match = index.Match("Phone or email");

        // assert
        match!.Key.Should().Be("Validation_PhoneOrEmail");
        index.GetFieldKey("Phone or email").Should().Be("Field_PhoneOrEmail");
    }
